...
def find_guard():
    ...
    for key,value in sleep_schedule.items():
        ...
        for i in range(0, len(value), 2):
            complete = complete + list(range(value[i], value[i + 1]))
        sleep_schedule[key] = complete

    minute_guard = {}
    for guard_id,pattern in sleep_schedule.items():
        for each_minute in pattern:
            minute_guard[each_minute] = {}

    for guard_id,pattern in sleep_schedule.items():
        guard_id = guard_id.split(" ")[-1]
        for each_minute in pattern:
            minute_guard[each_minute][guard_id] = minute_guard[each_minute].get(guard_id,0) + 1

    for key,value in minute_guard.items():
        frq = max(value.values())
        for id,freq in value.items():
            if freq == frq:
                minute_guard[key] = (id,freq)
